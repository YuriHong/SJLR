...
+ (id<TYAppendTextStorageProtocol>)parseTextStorageFromDictinary:(NSDictionary *)dic
{
    ...
    if (fontSize > 0) {
        textStorage.font = [UIFont fontWithName:@"HYQuanTangShiJ" size:fontSize];
    }
...
+ (id<TYDrawStorageProtocol>)parseImageRunFromDictinary:(NSDictionary *)dic
{
    ...
    imageStorage.imageURL = [NSURL URLWithString:dic[@"imageURL"]];
    imageStorage.placeholdImageName = dic[@"placeholdImageName"];
    imageStorage.imageAlignment = TYImageAlignmentCenter;
    imageStorage.size = CGSizeMake([UIScreen mainScreen].bounds.size.width, 200);
...
+ (id<TYAppendTextStorageProtocol>)parseLinkRunFromDictinary:(NSDictionary *)dic
{
    ...
    linkStorage.font = [UIFont fontWithName:@"HYQuanTangShiJ" size:[dic[@"size"] integerValue]];
