...
@interface BottomPickerView()<UIGestureRecognizerDelegate>
...
-(instancetype)initWithController:(UIViewController *)controller{
    ...
    if(self){
        ...
        //LHEditPickerView
        EditPickView *editView=[[EditPickView alloc]initWithFrame:CGRectMake(0, controller.view.bounds.size.height, controller.view.bounds.size.width, controller.view.bounds.size.height/5*2)];
...
+(instancetype)showWithController:(UIViewController *)controller andData:(NSArray *)data{
    BottomPickerView *bottom=[[self alloc]initWithController:controller];
...
+(void)showEditPickerViewWithController:(UIViewController *)controller andData:(NSArray *)data andBlock:(void (^)(NSString *temp))bottomResultPresent{
    BottomPickerView *bottom=[BottomPickerView showWithController:controller andData:data];
