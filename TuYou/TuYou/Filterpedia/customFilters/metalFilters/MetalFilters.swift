...
class MetalFilter: CIFilter, MetalRenderable
{
    ...
    func imageFromComputeShader(width: CGFloat, height: CGFloat, inputImage: CIImage?) -> CIImage
    {
        ...
        for inputKey in inputKeys {
            if let attris = (attributes[inputKey] as? [String: Any]), attris[kCIAttributeClass] as! String == "NSNumber" {
                ...
                {
//                    device.makeBuffer(bytes: &bufferValue, length: MemoryLayout<Float>.size, options: .cpu)
...
//        }
        ...
        for inputKey in inputKeys {
            if let attris = (attributes[inputKey] as? [String: Any]), attris[kCIAttributeClass] as! String == "CIColor" {
                if let bufferIndex = (attributes[inputKey] as! [String:AnyObject])[kCIAttributeIdentity] as? Int,
