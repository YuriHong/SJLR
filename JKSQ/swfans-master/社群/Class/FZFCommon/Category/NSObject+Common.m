...
+ (void)pingbiAuthor:(NSString *)authorID {
    NSUserDefaults *de = [NSUserDefaults standardUserDefaults];
    NSString *str = @"pingbi.jubao";
    NSArray *ids = (NSArray *)[de objectForKey:str];
    NSMutableArray *data = [NSMutableArray array];
    if (ids.count) {
        [data addObjectsFromArray:ids];
    }
    [data addObject:authorID];
    [de setObject:data forKey:str];
    [de synchronize];
}

+ (BOOL)checkAuthorID:(NSString *)authorID {
    NSUserDefaults *de = [NSUserDefaults standardUserDefaults];
    NSString *str = @"pingbi.jubao";
    NSArray *data = (NSArray *)[de objectForKey:str];
    return [data containsObject:authorID];
}
@end
