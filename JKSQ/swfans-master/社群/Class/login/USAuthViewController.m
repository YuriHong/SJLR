...
- (IBAction)clicktongyi:(id)sender {
    [self.eulaView loadTextFromFile:@"极客社区应用" type:@"txt"];
    [self showEula:YES];
}
...
- (USEULAView *)eulaView {
    if (!_eulaView) {
        _eulaView = [USEULAView eulaView];
        MJWeakSelf;
        [_eulaView configureAtion:^(NSInteger tag) {
            if (tag == 0) {
                
            }else {
                if (!weakSelf.logButton.enabled) {
                    protocolImageView.hidden = !protocolImageView.hidden;
                    weakSelf.logButton.enabled = !protocolImageView.hidden;
                }
            }
            weakSelf.eulaView.hidden = YES;
        }];
         _eulaView.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT);
        [self.view addSubview:_eulaView];
        _eulaView.hidden = YES;
    }
    return _eulaView;
}

- (void)showEula:(BOOL)show {
    self.eulaView.hidden = !show;
    [UIView animateWithDuration:0.25 animations:^{
        CGFloat y = show ? 0 : SCREEN_HEIGHT;
        _eulaView.frame = CGRectMake(0, y, SCREEN_WIDTH, SCREEN_HEIGHT);
    }];
}
